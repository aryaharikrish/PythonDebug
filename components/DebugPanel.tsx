...
import { AlertTriangle, CheckCircle2, Lightbulb, ArrowRight, Check, Copy, Code, Sparkles, Info, Power, BotOff } from "lucide-react";
...
export function DebugPanel({ 
  ...
  if (!enabled) {
    return (
      <div className="h-full bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-col items-center justify-center text-center space-y-4 shadow-xl">
        ...
        <div>
          <h3 className="text-base font-semibold text-slate-200">AI Assistance Disabled</h3>
          <p className="text-xs text-slate-400 mt-1 max-w-xs leading-relaxed">
            AI error diagnosis and automatic code repair are currently turned off.
          </p>
        </div>
        <button
          onClick={handleToggle}
          className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-xs font-semibold flex items-center gap-2 transition-all shadow-md active:scale-95"
        >
    ...
    return (
      <div className="h-full bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-col items-center justify-center text-center space-y-4 shadow-xl">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Inspecting code structure, tracebacks, and root causes</p>
        </div>
      </div>
    );
  }
  ...
  if (!hasExecuted) {
    return (
      <div className="h-full bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-col items-center justify-center text-center text-slate-400 space-y-3 shadow-xl">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleToggle}
            title="Disable AI Assistance"
            className="text-[11px] px-2 py-0.5 rounded bg-slate-800 hover:bg-red-950 hover:text-red-400 text-slate-400 border border-slate-700 transition-colors"
          >
            Turn Off
          </button>
        </div>
  ...
  if (!analysis || analysis.errorType === "None") {
    return (
      <div className="h-full bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-col items-center justify-center text-center space-y-3 shadow-xl">
  ...
  const hasCodeFix = analysis.correctedCode && analysis.correctedCode.trim().length > 0;
  ...
  return (
    <div className="h-full bg-slate-900 border border-slate-800 rounded-xl flex flex-col overflow-hidden shadow-xl">
      ...
      <div className="px-4 py-3 bg-red-950/40 border-b border-red-900/50 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleToggle}
            title="Disable AI Assistance"
            className="p-1 rounded hover:bg-red-900/40 text-slate-400 hover:text-red-300 transition-colors text-xs flex items-center gap-1 border border-slate-800"
          >
            <Power className="w-3.5 h-3.5" />
            <span>AI Off</span>
          </button>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 text-xs">
        ...
        {/* Optional Corrected Code Preview */}
        {hasCodeFix && (
          <div className="pt-2 border-t border-slate-800">
            <div className="flex items-center justify-between mb-2">
              <button
                onClick={() => setShowCodeFix(!showCodeFix)}
                className="text-xs font-medium text-slate-300 hover:text-blue-400 flex items-center gap-1.5 bg-slate-950 px-2.5 py-1 rounded border border-slate-800 transition-colors"
              >
                <Code className="w-3.5 h-3.5 text-blue-400" />
                <span>{showCodeFix ? "Hide Automated Code Preview" : "Show Automated Code Preview"}</span>
              </button>
            ...
            {showCodeFix && (
              <div className="space-y-2 mt-2">
                ...
                <button
                  ...
                  className="w-full py-2 px-4 rounded-lg bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-semibold text-xs shadow-md flex items-center justify-center gap-2 transition-all active:scale-[0.98]"
                >
                  <Sparkles className="w-4 h-4" />
                  Apply Code Preview to Editor
                </button>
              </div>
            )}
          </div>
